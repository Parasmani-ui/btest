...
interface PowerCrisisSimulationClientProps {
...
const PowerCrisisSimulationClient: React.FC<PowerCrisisSimulationClientProps> = ({ 
  simulationText, 
  onStartNewCase,
  onSessionStart,
  onSessionEnd
  ...
  useEffect(() => {
    if (simulationText) {
      ...
      setMessages([
        { role: 'system', content: 'Power Crisis Simulation started.' },
  ...
  useEffect(() => {
    if (simulationText && !sessionStarted && !sessionInitialized.current) {
      ...
      startSession('powercrisis').then(() => {
        setSessionStartTime(startTime);
        setSessionStarted(true);
        onSessionStart?.(startTime); // Notify parent of session start
        console.log('✅ Power crisis simulation session started when scenario loaded');
      }).catch(error => {
        console.error('❌ Error starting power crisis session:', error);
      });
    }
  ...
  const parseScenarioAndOptions = (content: string) => {
    ...
    if (content.includes('FINAL PERFORMANCE EVALUATION') || content.includes('Final Score:')) {
      ...
      if (!gameEndedRef.current) {
        ...
        const totalScore = calculatePowerCrisisScore(messages, currentRound);
        ...
        (async () => {
          ...
          try {
            const scoreCalc = calculateSimulationScore('POWERPLANT_CRISIS_SIMULATION', content, null, messages);
            await updateSession({
              analysis: content,
              caseTitle: `Power Crisis Simulation`,
              scoreBreakdown: {
          ...
          try {
            ...
            console.log('✅ Power crisis simulation stats updated successfully');
          } catch (error) {
  ...
  const handleSubmit = async (event: React.FormEvent | null, optionInput?: string) => {
    ...
    try {
      ...
      const response = await fetch('/api/power-crisis-simulation/interact', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        if (data.response.includes('FINAL PERFORMANCE EVALUATION') || data.response.includes('Final Score:')) {
          ...
          if (!gameEndedRef.current) {
            ...
            try {
              ...
              const scoreCalc = calculateSimulationScore('POWERPLANT_CRISIS_SIMULATION', aiMessageContent, null, messages);
              await updateSession({
            ...
            try {
              ...
              console.error('❌ Error updating power crisis simulation stats:', error);
            }
          } else {
            console.log('🔄 Game already ended, skipping duplicate handleGameEnd call');
          }
        ...
        console.error('Failed to get response from power crisis simulation API');
      }
    } catch (error) {
      console.error('Error in power crisis simulation:', error);
    } finally {
      setIsThinking(false);
    }
  };
    ...
    if (isScenarioWithOptions && isLatestAssistantMessage) {
      ...
      return (
        <div>
          ...
          {/* Options as buttons */}
          <div className="mt-6 px-4 py-3 bg-purple-100 dark:bg-purple-950 text-black dark:text-white border border-purple-500 border-opacity-20 rounded-lg">
  ...
  // Calculate 3-parameter score for power crisis simulation
  const calculatePowerCrisisScore = (messages: {role: string, content: string}[], currentRound: number): number => {
    const finalMessage = messages[messages.length - 1];
    if (finalMessage?.content) {
      // Use the new 3-parameter scoring system
      const score = calculateSimulationScore('POWERPLANT_CRISIS_SIMULATION', finalMessage.content, null, messages);
      ...
      <div className={`flex flex-col h-screen ${theme === 'dark' ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-900'}`}>
        ...
        <div className={`p-3 ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'} flex justify-between items-center shadow-sm border-b ${theme === 'dark' ? 'border-gray-700' : 'border-gray-200'}`}>
          <div className="flex items-center space-x-4">
            {role && (
              <div className={`px-3 py-1 rounded-full text-sm ${theme === 'dark' ? 'bg-gray-700 text-white' : 'bg-purple-100 text-purple-800'}`}>
                Role: {role}
              </div>
            )}
          ...
          <div className="max-w-4xl mx-auto space-y-4">
              ...
              return (
                ...
                >
                  <div className={`text-sm font-semibold mb-1 ${
                    ...
                    {message.role === 'assistant' ? 'ELECTRICRUX' : 'You'}
                  </div>
            ...
            {isThinking && (
              <div className={`p-4 rounded-lg max-w-3xl ${
                ...
                <span className="text-sm ml-2">ELECTRICRUX is thinking...</span>
              </div>
            )}
          </div>
        </div>
              ...
              <div className="space-y-4">
                {finalElapsedTime && (
                  <div className="text-center">
                    <div className={`text-lg font-semibold ${theme === 'dark' ? 'text-gray-300' : 'text-purple-600'}`}>
              ...
              <form onSubmit={(e) => handleSubmit(e)} className="flex space-x-2">
                <input
                  ...
                  className={`flex-1 p-3 rounded-lg ${
                    theme === 'dark' ? 'bg-gray-900 text-white border-purple-600' : 'bg-gray-100 text-gray-900 border-gray-300'
                ...
                <ShimmerButton
                  ...
                  shimmerDuration="2s"
                  borderRadius="0.5rem"
                  background={isThinking ? "rgb(156, 163, 175)" : "rgb(168, 85, 247)"}
...
export default PowerCrisisSimulationClient;
