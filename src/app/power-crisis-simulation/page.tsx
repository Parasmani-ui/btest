...
import PowerCrisisSimulationClient from './client-page';
...
export default function PowerCrisisSimulationPage() {
  ...
  const handleStartSimulation = () => {
    checkAuthAndProceed(generateSimulation, 'Power Crisis');
  };
  ...
  const generateSimulation = async () => {
    ...
    try {
      ...
      await startSession('powercrisis');

      // Update user stats when game starts (count games on start, not end)
      if (userData?.uid) {
        console.log(`📊 Updating user stats for game start: powercrisis`);
        await updateUserStatsOnGameStart(userData.uid, 'powercrisis');
    ...
    try {
      const response = await fetch('/api/power-crisis-simulation/generate', {
        method: 'POST',
        headers: {
  ...
  if (isLoading) {
    return (
      <ThemeProvider value={{ theme, toggleTheme }}>
        <GameHeader 
          gameTitle="Power Crisis Management" 
          showTimestamp={true} 
          startTiming={false}
          gameEnded={gameEnded}
        ...
        <div className={`flex min-h-screen items-center justify-center ${theme === 'dark' ? 'bg-gray-900' : 'bg-gray-100'}`}>
          <div className={`${theme === 'dark' ? 'bg-gray-800 text-white' : 'bg-white text-gray-800'} p-8 rounded-lg shadow-lg max-w-md w-full text-center`}>
            <TextAnimate
              ...
              duration={0.3}
            >
              Generating Power Crisis Simulation
            </TextAnimate>
            <TextAnimate
              ...
              duration={0.2}
            >
              Please wait while we create your power plant crisis scenario...
            </TextAnimate>
          ...
          <div className="max-w-4xl mx-auto px-8 py-4">
            ...
            <div className={`${theme === 'dark' ? 'bg-gray-800' : 'bg-white'} p-5 rounded-lg shadow-lg mb-4 transition hover:translate-y-[-5px]`}>
              <h1 className={`text-2xl font-bold ${theme === 'dark' ? 'text-white' : 'text-gray-800'} mb-3`}>
                Power Crisis Management Simulation
              </h1>
              <TextAnimate
                ...
                duration={0.3}
              >
                This simulation is designed for energy professionals who want to practice decision-making in high-pressure power plant crisis scenarios.
              </TextAnimate>
              <TextAnimate
            ...
            <div className={`${theme === 'dark' ? 'bg-gray-800' : 'bg-white'} p-5 rounded-lg shadow-lg transition hover:translate-y-[-5px]`}>
              ...
              <ul className={`${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'} space-y-2 text-sm`}>
                <li className="flex items-start transition hover:translate-x-1">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2 text-purple-500 flex-shrink-0 transition" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  <TextAnimate animation="slideUp" by="word" duration={0.2} delay={0.1}>
                    Each simulation consists of 10 connected rounds of power plant crisis scenarios.
                  </TextAnimate>
                </li>
                <li className="flex items-start transition hover:translate-x-1">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2 text-purple-500 flex-shrink-0 transition" fill="none" viewBox="0 0 24 24" stroke="currentColor">
