...
export async function POST(request: NextRequest) {
  console.log('Processing power crisis simulation interaction');
    ...
    try {
      const completion = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: POWERPLANT_CRISIS_SIMULATION_PROMPT,
          },
          {
            role: 'system',
      ...
      if (isRound10Complete) {
        ...
        formattedScores = formatFinalScores(calculatedScores, 'POWERPLANT_CRISIS_SIMULATION');
