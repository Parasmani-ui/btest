...
const FinancialNegotiationClient: React.FC<FinancialNegotiationClientProps> = ({
  simulationText,
  subGameIndex,
  onStartNewCase,
  onBackToSelection,
  ...
  const [isGeneratingNewCase, setIsGeneratingNewCase] = useState<boolean>(false);
  ...
  useEffect(() => {
    if (simulationText) {
      ...
      setMessages([
        { role: 'system', content: 'Financial Investigation Simulation started.' },
  ...
  useEffect(() => {
    if (simulationText && !sessionStarted && !sessionInitialized.current) {
      ...
      startSession('financial-negotiation').then(() => {  // Using 'financial-negotiation' now that it's a valid type
  ...
  const parseScenarioAndOptions = (content: string) => {
    ...
    // Extract investigation progress if available
    const progressMatch = content.match(/Progress: ([^\n]+)/);
    if (progressMatch && progressMatch[1]) {
      setInvestigationProgress(progressMatch[1].trim());
    }
    ...
    if (content.includes('FINAL PERFORMANCE EVALUATION') ||
        ...
        content.includes('INVESTIGATION COMPLETE') ||
  ...
  const handleSubmit = async (event: React.FormEvent | null, optionInput?: string) => {
    ...
    try {
      ...
      const response = await fetch('/api/financial-negotiation/interact', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        const isFinal = data.isComplete || 
                        ...
                        data.response.includes('INVESTIGATION COMPLETE') ||
  ...
  const handleHint = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        setHintText(data.hint || 'Focus on asking specific questions about documentation and cross-verifying information.');
        setShowHint(true);
        setHintUsed(true);
      } else {
  ...
  // Handle starting a new case (same sub-game)
  const handleStartNewCase = async () => {
    setIsGeneratingNewCase(true);
    
    try {
      // Generate a new case for the same sub-game
      ...
      if (response.ok) {
        ...
        // Reset all states for new case
        setIsCompleted(false);
        setGameStarted(true);
        setMessages([
          { role: 'system', content: 'Financial Investigation Simulation started.' },
          { role: 'assistant', content: data.simulation }
        ]);
        setCurrentTurn(1);
        setRole('');
        setScenarioText('');
        ...
        parseScenarioAndOptions(data.simulation);
        
        // Start new session
        const startTime = new Date();
        await startSession('financial-negotiation');
        setSessionStartTime(startTime);
        setSessionStarted(true);
        setCurrentElapsedTime('0m 0s');
        sessionInitialized.current = true;
        ...
        <div>
          ...
          <div className={`mt-6 p-6 border border-opacity-30 rounded-xl ${
            ...
            <p className="font-semibold mb-4 text-xl flex items-center">
              ...
              Select Your Investigation Approach
            </p>
            ...
            <div className="mt-4 text-sm opacity-70 text-center">
              Select an option above or type your own investigation approach below.
            </div>
          </div>
        </div>
      );
            ...
            <form onSubmit={handleSubmit} className="flex flex-col space-y-4">
              <div className="flex space-x-4">
                <textarea
                  ...
                  placeholder={selectedOption ? `Option ${selectedOption} selected. Click Send or type your own approach...` : "Type your investigation approach or response... (Type 'exit' to end early)"}
                  disabled={isThinking}
                  rows={1}
           ...
           <div className="flex flex-col items-center space-y-3">
             ...
             <div className="flex space-x-3">
               <ShimmerButton
                 ...
                 } text-white ${isGeneratingNewCase ? 'opacity-50 cursor-not-allowed' : ''}`}
               >
                 {isGeneratingNewCase ? (
                   <div className="flex items-center space-x-2">
                     <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div>
...
export default FinancialNegotiationClient; 